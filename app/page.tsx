...
//           setIsInMatch(false)
//           setQueueStartTime(null)
//           // Don't automatically show game selection to prevent UI jump
...
//         console.log("WebSocket reconnected")
//         setWsStatus("Connected")
//         // Send queue command after successful reconnection
//         if (selectedGames.length > 0) {
...
export default function PlayPage() {
  ...
  function handleQueueClick() {
    ...
    // setShowGameSelection(false)
    // ws.send(JSON.stringify({ command: "queue", environments: selectedGames }))
    // We’re now fully reset, so the queue overlay can show.
    ws.send(JSON.stringify({ command: "queue", environments: selectedGames }));
    setShowGameSelection(false); 
  }
